...
import { getSourceInvoiceById } from 'apiSdk/source-invoices';
import { SourceInvoiceInterface } from 'interfaces/source-invoice';

function SourceInvoiceViewPage() {
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('source_invoice', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/source-invoices/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                ...
                {hasAccess('source', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Source"
                    text={
                      <Link as={NextLink} href={`/sources/view/${data?.source?.id}`}>
                        {data?.source?.name}
                      </Link>
                    }
                  />
