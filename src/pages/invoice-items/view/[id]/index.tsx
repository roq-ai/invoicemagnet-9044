...
import { getInvoiceItemById } from 'apiSdk/invoice-items';
import { InvoiceItemInterface } from 'interfaces/invoice-item';

function InvoiceItemViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<InvoiceItemInterface>(
    () => (id ? `/invoice-items/${id}` : null),
    () =>
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('invoice_item', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/invoice-items/edit/${id}`} passHref legacyBehavior>
                    <Button
