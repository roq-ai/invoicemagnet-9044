...
import { getInvoiceItems } from 'apiSdk/invoice-items';
import { InvoiceItemInterface } from 'interfaces/invoice-item';
import { getSourceInvoices } from 'apiSdk/source-invoices';
...
import { SourceListPage as ShowCaseList } from 'pages/sources';

function DashboardPage() {
  const { session } = useSession();

  const invoiceItemStat = useSWR<PaginatedInterface<InvoiceItemInterface>>(
    () => `/invoice-items`,
    () => getInvoiceItems({ limit: 1, offset: 0 }),
  );

  const sourceInvoiceStat = useSWR<PaginatedInterface<SourceInvoiceInterface>>(
    () => `/source-invoices`,
    () => getSourceInvoices({ limit: 1, offset: 0 }),
  );

  const sourceStat = useSWR<PaginatedInterface<SourceInterface>>(
    () => `/sources`,
    () => getSources({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Invoice Items',
      result: invoiceItemStat?.data?.totalCount,
      isLoading: invoiceItemStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Source Invoices',
      result: sourceInvoiceStat?.data?.totalCount,
      isLoading: sourceInvoiceStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Sources',
      result: sourceStat?.data?.totalCount,
      isLoading: sourceStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
